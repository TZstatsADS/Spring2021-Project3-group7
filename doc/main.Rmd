...
```
Run test images for Random Forest
```{r test random forest }
tm_test_rf = NA
feature_test <- as.matrix(dat_test[, -6007])
if(run.test){
  load(file="../output/fit_train_xgb.RData")
  tm_test_rf <- system.time({label_pred_rf <- predict(fit_train_rf, feature_test, pred.type = 'class');
                          prob_pred_rf <- predict(fit_train_rf, feature_test, pred.type = 'prob')})
} 
...
accu_rf <- mean((label_pred_rf == label_test_rf))
tpr.fpr_xgb <- WeightedROC(prob_pred_rf, label_test_rf)
auc_xgb <- WeightedAUC(tpr.fpr_rf)
